...
## 项目成果
- **一个项目**，9 份交付物闭环（需求→计划→用例→环境→抓包→缺陷→报告）+ 自动化 + CI，可一键复现。
- 接口 21 用例（17 passed + 4 xfailed）、移动端 6 用例真机全绿；Charles 实证 3 个预埋缺陷；Jenkins 6 场景落地。
- 交付资产：Jenkinsfile + 4 变体 + `docs/10-Jenkins教学.md` + 单命令启动脚本（`adb connect` → 起服务 → 起 Appium → pytest`）。
